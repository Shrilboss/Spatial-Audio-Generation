...
class LatentDiffusion(DDPM):
    ...
    #     audio_features,text_features,audio_features_mlp,text_features_mlp,audio_logits,text_logits = self.cond_stage_model.model(audio,text)
    
    def forward(self, x, c, *args, **kwargs):
        if kwargs.get('validation', False):
            t = (self.num_timesteps//2) * torch.ones((x.shape[0],), dtype=torch.int64, device=self.device)
        else:
